...
export async function getProfileByProfileActivationToken(profileActivationToken: string) {
	try {
		...
		const [rows] = await mysqlConnection.execute('SELECT BIN_TO_UUID(profileId) as profileId, profileActivationToken,' +
			' profileFirstName, profileLastName, profileLanguage, profileEmailAddress, profileImage ' +
			'FROM profile WHERE profileActivationToken = :profileActivationToken', {profileActivationToken});
